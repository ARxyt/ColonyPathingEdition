...
public class NewFoodUtils {
    ...
    public static void consumeFood(final ItemStack foodStack, final AbstractEntityCitizen citizen, @org.jetbrains.annotations.Nullable final Player player)
    {
        final ICitizenData citizenData = citizen.getCitizenData();
        final double satIncrease = getFoodValue(foodStack, citizen);
        citizenData.increaseSaturation(satIncrease);

        ItemStack itemUseReturn = consumeFoodStack(foodStack, citizen);
    ...
    public static double getFoodValue(final ItemStack foodStack, @Nullable final FoodProperties itemFood, final double researchBonus)
    {
        ...
        final double nutrition = itemFood.nutrition() * (foodStack.getItem() instanceof IMinecoloniesFoodItem ? foodPunishMinecolonies : foodPunishNormal);
        final double bonus = itemFood.saturation() * (foodStack.getItem() instanceof IMinecoloniesFoodItem ? foodBonusMinecolonies : foodBonusNormal) * (1.0 + researchBonus);
