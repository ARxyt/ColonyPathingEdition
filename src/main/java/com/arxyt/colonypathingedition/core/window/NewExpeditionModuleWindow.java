...
public class NewExpeditionModuleWindow extends AbstractModuleWindow<ExpeditionLogModuleView>
{
    /**
    ...
    public NewExpeditionModuleWindow(@NotNull final ExpeditionLogModuleView module)
    {
        super(module, ResourceLocation.fromNamespaceAndPath(Constants.MOD_ID, "gui/layouthuts/layoutexpeditionlog.xml"));
    }
    ...
    private void createMobList(View mobsView, List<Tuple<EntityType<?>, Integer>> mobs)
    {
        ...
        final int MOB_SIZE = Math.min (24, mobsView.getWidth() / (size == 0 ? 1 : size));
                                ...
                                @NotNull final List<ItemStorage> loot)
    {
        ...
        final int rows = Math.max((int)Math.round(Math.sqrt(size / 2.0D)), 1);
        final int columns = Math.max((int)Math.ceil(size / (double)rows), 1);

        final int LOOT_GRID = Math.min(Math.min(lootView.getInteriorWidth() / columns, lootView.getInteriorHeight() / rows), 18);
        final int LOOT_SIZE = LOOT_GRID - 2;
