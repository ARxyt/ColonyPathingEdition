...
import static com.arxyt.colonypathingedition.core.costants.AdditionalContants.STUCK_DELAY;
...
public class NewEntityAIWorkPlanter extends AbstractEntityAICrafting<JobPlanter, BuildingPlantation>
{
    /**
     ...
     */
    public NewEntityAIWorkPlanter(@NotNull final JobPlanter job)
    {
        super(job);
        super.registerTargets(
                new AITarget(PREPARING, this::prepare, 1),
                new AITarget(PLANTATION_PICK_FIELD, this::pickField, 1),
                new AITarget(PLANTATION_DECIDE_FIELD_WORK, this::decideFieldWork, 1),
                new AITarget(PLANTATION_WORK_FIELD, this::workField, 1),
                new AITarget(PLANTATION_RETURN_TO_BUILDING, this::returnToBuilding, 1));
    ...
    private IAIState workField()
    {
        ...
        NewEntityAIWorkPlanter.ActionHandlerResult handlerResult = switch (activeModuleResult.getAction())
        {
            case NONE -> NewEntityAIWorkPlanter.ActionHandlerResult.FINISHED;
        ...
        if (handlerResult.equals(NewEntityAIWorkPlanter.ActionHandlerResult.FINISHED))
        {
        ...
        else if (handlerResult.equals(NewEntityAIWorkPlanter.ActionHandlerResult.NEEDS_ITEM))
        {
            activeModuleResult = null;
     ...
     */
    private NewEntityAIWorkPlanter.ActionHandlerResult handlePlantingAction()
    {
        if (!Objects.isNull(activeModuleResult.getActionPosition()))
        {
            ...
            {
                return NewEntityAIWorkPlanter.ActionHandlerResult.NEEDS_ITEM;
            }
            ...
            if (world.setBlockAndUpdate(activeModuleResult.getActionPosition(), blockState))
            {
                ...
                CitizenItemUtils.removeHeldItem(worker);
                return NewEntityAIWorkPlanter.ActionHandlerResult.FINISHED;
            }

            return NewEntityAIWorkPlanter.ActionHandlerResult.BUSY;
        }
        return NewEntityAIWorkPlanter.ActionHandlerResult.FINISHED;
    }
     ...
     */
    private NewEntityAIWorkPlanter.ActionHandlerResult handleBonemealAction()
    {
        if (!Objects.isNull(activeModuleResult.getActionPosition()))
        {
            IPlantationModule planterModule = activeModuleResult.getModule();
     ...
     */
    private NewEntityAIWorkPlanter.ActionHandlerResult handleMiningAction(boolean isHarvest)
    {
        if (!Objects.isNull(activeModuleResult.getActionPosition()))
        {
            ...
            return mineResult ? NewEntityAIWorkPlanter.ActionHandlerResult.FINISHED : NewEntityAIWorkPlanter.ActionHandlerResult.BUSY;
        }
        return NewEntityAIWorkPlanter.ActionHandlerResult.FINISHED;
    }
