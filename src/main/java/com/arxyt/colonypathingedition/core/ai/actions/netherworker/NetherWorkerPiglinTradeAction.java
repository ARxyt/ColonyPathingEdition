...
public class NetherWorkerPiglinTradeAction extends AdventureActionHandler.Action {
    ...
    public NetherWorkerPiglinTradeAction(Level world, AbstractEntityCitizen worker, JobNetherWorker job, CompoundTag tag){
        ...
        if(tag.contains("tradeLoot")) {
            this.lootTableKey = ResourceKey.create(Registries.LOOT_TABLE, ResourceLocation.parse(tag.getString("tradeLoot")));
        }
        if(tag.contains("cost")) {
            ResourceLocation itemResourceLocation = ResourceLocation.parse(tag.getString("cost"));
            Item costItem = BuiltInRegistries.ITEM.get(itemResourceLocation);
        ...
        if(tag.contains("optionalCost")) {
            ResourceLocation itemResourceLocation = ResourceLocation.parse(tag.getString("optionalCost"));
            Item costItem = BuiltInRegistries.ITEM.get(itemResourceLocation);
            if(costItem != Items.AIR) {
                int costCount = 1;
    ...
    protected AdventureActionHandler.ResultType tick() {
        ...
        final LootTable loot = Objects.requireNonNull(world.getServer()).reloadableRegistries().getLootTable(lootTableKey);
