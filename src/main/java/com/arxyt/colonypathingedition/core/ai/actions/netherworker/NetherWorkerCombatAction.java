...
import static com.arxyt.colonypathingedition.core.ai.actions.handler.AdventureActionHandler.ActionType.COMBAT;
import static com.arxyt.colonypathingedition.core.ai.actions.handler.AdventureActionHandler.ActionType.NONE;
...
public class NetherWorkerCombatAction extends AdventureActionHandler.Action {
    ...
    public NetherWorkerCombatAction(Level world, AbstractEntityCitizen worker, AbstractJob<?, ?> job, AdventureData adventureData, boolean extraRound) {
        ...
        this.mobType = adventureData.entityType();
        this.mobDamage = adventureData.damage();
    ...
    protected AdventureActionHandler.ResultType tick() {
        ...
        if(mobHealth < 0 ) {
            ...
            LootTable loot = Objects.requireNonNull(world.getServer()).reloadableRegistries().getLootTable(mob.getLootTable());
        ...
        if(!worker.isDeadOrDying()) {
            ...
            if (!sword.isEmpty())
            {
                ...
                {
                    damageToDo += sword.getItem().getDamage(sword);
                }
                else
                {
                ...
                sword.hurtAndBreak(1, worker, EquipmentSlot.MAINHAND);
            }
