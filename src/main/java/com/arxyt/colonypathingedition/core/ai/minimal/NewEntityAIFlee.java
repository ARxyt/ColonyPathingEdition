...
public class NewEntityAIFlee implements IStateAI {
    ...
    public IState isEntityClose()
    {
        ...
        {
            distanceFromEntity = citizen.distanceTo(currentClosest);
    ...
    private Entity getClosestToAvoid()
    {
        ...
        {
            float healthMultiplyer = 1 + (citizen.getMaxHealth() - citizen.getHealth()) / citizen.getMaxHealth();
    ...
    private boolean updateMoving()
    {
        ...
        {
            if (closestLivingEntity == null || citizen.distanceTo(closestLivingEntity) > MOVE_AWAY_DIST)
            {
